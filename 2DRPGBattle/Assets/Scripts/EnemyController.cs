...
public class EnemyController : MonoBehaviour
{
    ...
    public IEnumerator ExecuteAttack(GameObject player)
    {
        ...
        {
            yield return StartCoroutine(PerformAttack(player, "attack2", minDmg2, maxDmg2, critChance2));
        }
    ...
    private IEnumerator PerformAttack(GameObject player, string trigger, int minDmg, int maxDmg, int critChance)
    {
        ...
        Debug.Log($"{enemyID} deals {damage} damage{(isCrit ? " (CRIT!)" : "")}");
